package org.primeit.governmentholidays.utils.toggle.widget;
...
public class LabeledSwitch extends ToggleableView {
    ...
    private void initView() {
        ...
        if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.M) {
            colorBorder = colorOn = getResources().getColor(R.color.primary_color, getContext().getTheme());
        } else {
            colorBorder = colorOn = getResources().getColor(R.color.primary_color);
        }
    ...
    private void initProperties(AttributeSet attrs) {
        ...
        for (int i = 0; i < N; ++i) {
            ...
            if (attr == R.styleable.Toggle_on) {
                ...
                int accentColor;
                if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.M) {
                    accentColor = getResources().getColor(R.color.primary_color, getContext().getTheme());
                } else {
                    accentColor = getResources().getColor(R.color.primary_color);
                }
